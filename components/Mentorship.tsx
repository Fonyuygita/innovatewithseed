...
const Mentorship = () => {
    ...
    return (
        ...
        <section className="w-full   py-[3rem] overflow-hidden  items-center  gap-6 flex-col">
            ...
            <div className={`h-[350px] w-screen md:w-[80%] lg:w-[76%] shadow-xl pt-4 mx-auto mb-[4rem] ${theme === 'light' ? 'text-gray-600 bg-blue-500  ' : ' text-light-300 bg-gray-900'}`}>
            ...
            >
                {MentorshipData.map((item, index) => (
                    <SwiperSlide className={`md:shadow-2xl min-h-[300px] md:h-[350px] w-[89%] px-9 md:w-full mb-[4rem] mt-[5rem] relative ${theme === 'light' ? 'text-gray-800 bg-blue-500  ' : ' text-light-300 bg-gray-900'}`} key={item.id}>
                        <div className="flex justify-between items-center px-5 w-full shadow-xl py-3">
