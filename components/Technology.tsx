...
const Technology = ({ title, image, description1, description2, description3, title1, title2, title3 }: TechProps) => {
    return (
        <div className='flex items-start justify-center gap-4 flex-col w-full p-[23px] min-h-[60vh] overflow-hidden'>
            <h2 className='text-bold text-4xl font-sans text-left mt-6 text-blue-950'>{title}</h2>
            <div className="w-full h-full p-6 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-950 to-blue-900 shadow-lg rounded-lg">
                <Image src={image} alt={title} width={340} height={300} className='object-contain w-full h-[80%] ' />
            </div>
